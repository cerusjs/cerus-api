...
module.exports = class json {
	 ...
	 * it'll return the header string that matches that code and when you don't insert anything 
	 ...
	 * @param {Number} (code) The status code you want the header string for.
	 * @return {String|Object} The header that matches the status code or all of the headers.
	 * @function header
	 */
	header(code) {
		if(typeof code !== "number") return headers;
	...
	emit() {
		...
		this._emitted = true;
		this._res.type("json");
		this._res.status(this._res.status() === 200 ? this.code() : undefined);
		this._res.json(this._json, {pretty: this._pretty});
	}
