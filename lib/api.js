...
class api {
	...
	load(name, type = "api") {
		...
		switch(type) {
			...
			case "policy":
				return require(this._cerus.root() + this._folders._policies + "/" + name_);

			// If type is api, load an api class
			case "api":
			default:
				return require(this._cerus.root() + this._folders._api + "/" + name_);
		}
	}
...
/**
 * This is the urls class. With this class you can change the url the api is routed to.
 * @class api.urls
 * @id api.urls
 */
class urls {
